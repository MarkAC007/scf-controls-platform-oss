...
TENANT_SCOPED_EXCLUDED_TABLES: Dict[str, str] = {
    ...
    # Evidence storage configuration: an encrypted object-store credential plus
    # an endpoint. The ciphertext is only readable under the source
    # deployment's SCF_SECRET_KEY, and the endpoint names a bucket the
    # destination may have no right to reach, so carrying it would either
    # arrive undecryptable or point a second deployment at the first one's
    # store. The receiving deployment configures its own.
    "evidence_storage_configs": "encrypted storage credential; deployment-local",
