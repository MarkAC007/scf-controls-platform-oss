...
class PostgresFTSBackend:
    ...
    def build_statement(
        ...
        *,
        ...
        # False positive: nothing user-controlled is interpolated here. Every
        # `{...}` above expands to either a bind-parameter NAME generated
        # server-side (`f"q{index}"` from `enumerate(query_texts)`) or the
        # module constant `_TS_RANK_NORMALISATION`. The query TEXT itself is
        # bound via `params[key]`, never formatted into the SQL. Rewriting this
        # as a static string is not possible: the number of ts_rank terms is a
        # function of len(query_texts), which is only known at call time.
