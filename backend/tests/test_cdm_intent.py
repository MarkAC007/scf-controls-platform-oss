...
def test_gate_allows_a_classified_document_with_no_surviving_intent_row():
    ...
    ON THE FAIL-OPEN PATH ONLY (this test's path). Fail-closed
    (require_defined_intent=True) deliberately excludes this edge: see
    test_fail_closed_stale_without_intent_rows_is_excluded in
    test_cdm_proposal_precision.py.
    ...
    every domain — the one outcome the fail-open gate exists to prevent.
    """
    session = _GateSession([
        _row(DOC_A, "classified", TEST_DOMAIN),
