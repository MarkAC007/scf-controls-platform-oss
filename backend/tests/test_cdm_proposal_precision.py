...
def test_summary_positional_construction_unchanged():
    ...
    assert summary.documents_excluded_unclassified == 0
...
def test_fail_closed_unclassified_is_excluded_everywhere():
    """An abstention proposes nowhere, not everywhere.

    Reversal of the original #712 allow: one abstained SOP produced 2,516
    cross-domain proposals (75% of a compute run). If the platform cannot
    say where a document belongs, it must not guess "everywhere".
    """
    doc_unclassified, doc_classified = uuid4(), uuid4()
    gate = _gate(
        [
            _row(doc_unclassified, "unclassified"),
            _row(doc_classified, "classified", TEST_DOMAIN),
        ],
        require_defined_intent=True,
    )

    assert doc_unclassified not in gate.allowed_documents(TEST_DOMAIN)
    assert doc_unclassified not in gate.allowed_documents(OTHER_DOMAIN)
    assert doc_unclassified not in gate.allowed_documents(None)
    assert gate.documents_excluded_unclassified == 1
    assert gate.documents_excluded_awaiting_intent == 0


def test_fail_closed_all_unclassified_corpus_allows_nothing():
    """All-abstained corpus yields empty sets — zero proposals, never None."""
    gate = _gate(
        [_row(uuid4(), "unclassified"), _row(uuid4(), "unclassified")],
        require_defined_intent=True,
    )

    assert gate.allowed_documents(TEST_DOMAIN) == set()
    assert gate.documents_excluded_unclassified == 2


def test_fail_open_unclassified_still_allowed():
    """Provider disabled: abstention keeps v2 permission semantics."""
    doc_a, doc_b = uuid4(), uuid4()
    gate = _gate(
        [
            _row(doc_a, "unclassified"),
            _row(doc_b, "classified", TEST_DOMAIN),
        ],
        require_defined_intent=False,
    )

    assert doc_a in gate.allowed_documents(OTHER_DOMAIN)
    assert gate.documents_excluded_unclassified == 0
...
def test_fail_closed_stale_without_intent_rows_is_excluded():
    """Fail-closed, unplaceable is indistinguishable from an abstention:
    a domain-scoped status with no surviving intent row proposes nowhere.
    (Fail-open keeps the original allowed-everywhere invariant.)"""
    doc_stale = uuid4()
    gate = _gate([_row(doc_stale, "stale")], require_defined_intent=True)

    assert doc_stale not in gate.allowed_documents(TEST_DOMAIN)
    assert gate.documents_excluded_unclassified == 1
...
def test_unclassified_document_computes_nothing_and_is_counted(monkeypatch):
    """Compute-level pin for the abstention exclusion: hits from an
    unclassified document are gate-skipped, and the summary carries the
    document in documents_excluded_unclassified — distinct from the
    awaiting-intent counter."""
    _score_is_ts_rank(monkeypatch)

    gate = _gate(
        [
            _row(DOC_ID, "classified", TEST_DOMAIN),
            _row(DOC_PENDING_ID, "unclassified"),
        ],
        require_defined_intent=True,
    )
    session = _FakeSession()
    ...
    assert session.added[0].cdm_document_id == DOC_ID
    assert summary.mappings_skipped_by_intent_gate == 1
    assert summary.documents_excluded_unclassified == 1
    assert summary.documents_excluded_awaiting_intent == 0
