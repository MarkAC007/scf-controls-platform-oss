/**
 * VendorDetailPage — single-scroll vendor detail per VendorDetail.html spec.
 *
 * Promotes VendorDetail.tsx's four-tab (overview/assess/decide/review) layout
 * to a continuous scrolling page. All content and actions are preserved.
 *
 * Layout (top to bottom):
 *   1. Breadcrumb + "k of N" pager strip
 *   2. Header block: accent bar + name + chips + RAG risk pill + rec pill + action buttons
 *   3. Data-access / meta line: data_classification + provenance + systems count
 *   4. Assessment state machine: in-progress / failed / completed report / empty
 *   5. Recommendation + conditions (from Decide tab)
 *   6. Action Items (VendorActionItemsPanel)
 *   7. Compensating Controls (VendorCompensatingControlsPanel)
 *   8. Assessment History (from Review tab)
 *   9. Next Review (from Review tab)
 *  10. Vendor Details card (from Overview tab — parity)
 *  11. Contact card (from Overview tab — parity)
 *  12. Contract card (from Overview tab — parity)
 *  13. Certifications table (from Overview tab — parity)
 *  14. Systems table (from Overview tab — parity)
 *
 * Keyboard: ArrowLeft→prev, ArrowRight→next, Escape→back.
 * Suppressed when focus is in input/textarea/select/contentEditable or a
 * listbox/combobox dropdown is open.
 */
...
function isSuppressed(e: KeyboardEvent): boolean {
  ...
  // Suppress if a listbox/combobox dropdown is open (dropdown guard)
...
export interface VendorDetailPageProps {
  ...
  vendorId: string
  /** Current filtered+sorted vendor list from VendorRegistry (for pager). */
  filteredVendors: Vendor[]
  /** Called when user opens/closes/pages. App owns push/replace decision. */
  onVendorItemChange: (id: string | null) => void
  onEdit: (vendor: Vendor) => void
  onDelete: (vendor: Vendor) => void
}

// ─── Shared inline styles (verbatim from VendorDetail.tsx) ────────────────────
...
export default function VendorDetailPage({
  ...
  // ── Pager ──────────────────────────────────────────────────────────────────
  const currentIndex = filteredVendors.findIndex(v => v.id === vendorId)
  const total = filteredVendors.length
  // currentIndex === -1 means not in filtered set → show "— of N"
  const pagerIndex = currentIndex === -1 ? null : currentIndex

  const handlePrev = useCallback(() => {
    if (currentIndex <= 0) return
    onVendorItemChange(filteredVendors[currentIndex - 1].id)
  }, [currentIndex, filteredVendors, onVendorItemChange])

  const handleNext = useCallback(() => {
    if (currentIndex === -1 || currentIndex >= filteredVendors.length - 1) return
    onVendorItemChange(filteredVendors[currentIndex + 1].id)
  }, [currentIndex, filteredVendors, onVendorItemChange])
  ...
  const handleAssessmentStarted = useCallback(() => {
    ...
    setPollError(null)
    // No setActiveTab call — single page, no tabs
  ...
  // ── Render ─────────────────────────────────────────────────────────────────
  return (
    <div className="vendor-detail-page" style={{ padding: '1.5rem' }}>

      {/* ================================================================
          Breadcrumb + pager strip
          ================================================================ */}
      <div
        style={{
          display: 'flex',
        ...
        <div style={{ marginLeft: 'auto', display: 'flex', alignItems: 'center', gap: '0.875rem' }}>
          {total > 0 && (
            <span style={{ fontSize: '0.8125rem', color: 'var(--muted)' }}>
              {pagerIndex !== null ? `${pagerIndex + 1} of ${total} vendors` : `— of ${total} vendors`}
            </span>
          )}
          <div
            style={{
          ...
          >
            ...
            <button
              onClick={handleNext}
              disabled={currentIndex === -1 || currentIndex >= filteredVendors.length - 1}
              aria-label="next"
              style={{
                padding: '0.4375rem 0.75rem',
                background: 'none',
                border: 'none',
                cursor: (currentIndex === -1 || currentIndex >= filteredVendors.length - 1) ? 'not-allowed' : 'pointer',
                opacity: (currentIndex === -1 || currentIndex >= filteredVendors.length - 1) ? 0.4 : 1,
              }}
            >
              <svg width="12" height="12" viewBox="0 0 14 14" fill="none" aria-hidden="true">
      ...
      {/* ================================================================
          Header block — name + chips + RAG + rec + action buttons
          ================================================================ */}
      <div
        style={{
          display: 'flex',
      ...
      >
        <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem', flex: '1 1 16rem', minWidth: 0 }}>
          {/* Name row */}
          ...
          {(assessedDate || reviewDue) && (
            <div style={{ marginLeft: '1rem', fontSize: '0.75rem', color: 'var(--muted)' }}>
              ...
              {reviewDue && (
                <span
                  style={{
                    color: reviewColour,
                    fontWeight: reviewStatus === 'ok' || !reviewStatus ? 400 : 600,
                  }}
                >
      ...
      >
        ...
        {systems.length > 0 && (
          <span style={{ marginLeft: vendor.data_classification ? '0.5rem' : 0 }}>
            {systems.length} system{systems.length !== 1 ? 's' : ''} linked
          </span>
        )}
      </div>
      ...
      {/* ================================================================
          ...
          ================================================================ */}
      {pollError && (
        <section style={{ ...cardStyle, borderColor: 'var(--destructive-border, #fecaca)', marginBottom: '1.5rem' }}>
      ...
      {/* ================================================================
          Recommendation + conditions (from Decide tab)
          ================================================================ */}
      ...
      {/* ================================================================
          Compensating Controls (from Decide tab)
          ================================================================ */}
      <section style={cardStyle}>
      ...
      {/* ================================================================
          Assessment History (from Review tab)
          ================================================================ */}
      <section style={cardStyle}>
                    ...
                    <tr key={assessment.id} style={{ borderBottom: '1px solid var(--border)' }}>
                      ...
                      <td
                        style={{
                          ...
                          fontWeight: assessment.final_risk_score != null ? 600 : 400,
                        }}
                      >
      ...
      {/* ================================================================
          Vendor Details (from Overview tab — parity)
          ================================================================ */}
      <section style={cardStyle}>
      ...
      {/* ================================================================
          Contact (from Overview tab — parity)
          ================================================================ */}
      <section style={cardStyle}>
      ...
      {/* ================================================================
          Contract (from Overview tab — parity)
          ================================================================ */}
      <section style={cardStyle}>
      ...
      {/* ================================================================
          Certifications (from Overview tab — parity)
          ================================================================ */}
      <section style={cardStyle}>
      ...
      {/* ================================================================
          Systems (from Overview tab — parity)
          ================================================================ */}
      <section style={cardStyle}>
